...
export class BasketListView extends List<IBasketItem> implements IBasketListView {
  ...
  protected renderItem(item: IBasketItem, index: number): HTMLElement {
    ...
    const deleteButton = element.querySelector<HTMLButtonElement>('.basket__item-delete');
    if (deleteButton) {
