...
export class BasketModal extends Modal implements IBasketModal {
	...
	const listContainer = basketElement.querySelector('.basket__list') as HTMLElement;
    if (!listContainer) throw new Error('Basket list container not found');

    this.totalPriceElement = basketElement.querySelector('.basket__price') as HTMLElement;
    if (!this.totalPriceElement) throw new Error('Basket total price element not found');

    this.submitButton = basketElement.querySelector('.basket__button') as HTMLButtonElement;
    if (!this.submitButton) throw new Error('Basket submit button not found in template');
