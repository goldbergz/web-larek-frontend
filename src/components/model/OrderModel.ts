...
import {
	...
	OrderStep,
	ValidationError,
} from '../../types/model/OrderModel';
import { Model } from '../base/Model';

export class OrderModel extends Model<IOrderState> implements IOrderModel {
	constructor() {
		super({
			step: 'payment',
	...
	getOrderData(): Order {
		...
		return {
			items: state.items.map((i) => i.product.id),
	...
	private removeErrors(fields: string[]): ValidationError[] {
		return this.state.validationErrors.filter((e) => !fields.includes(e.field));
	}
}
