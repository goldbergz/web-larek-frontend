...
export class BasketModel extends Model<IBasketState> implements IBasketModel {
	constructor() {
		super({
			items: [],
			totalQuantity: 0,
	...
	addProduct(product: IProduct): void {
		const exists = this.state.items.some((i) => i.product.id === product.id);
	...
	removeProduct(productId: string): void {
		const items = this.state.items.filter((i) => i.product.id !== productId);
